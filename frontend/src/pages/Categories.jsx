          ...
          {categories.map((cat) => {
            ...
            return (
              <div
                ...
                className={`group rounded-3xl p-6 bg-gradient-to-br ${cat.bgGradient} border ${cat.borderColor} shadow-soft hover:shadow-card-hover transition-all duration-300 cursor-pointer flex flex-col justify-between`}
              >
                <div>
                  <div className="flex items-center justify-between mb-4">
                    <span className="px-3 py-1 bg-white/80 backdrop-blur-xs text-slate-700 rounded-full text-xs font-bold shadow-xs">
                      {cat.itemCount}
                    </span>
                    <div className="w-9 h-9 rounded-full bg-white text-slate-600 group-hover:bg-brand-500 group-hover:text-white flex items-center justify-center transition-colors shadow-xs">
                      <ArrowRight className="w-4 h-4 stroke-[2.5]" />
                    </div>
                  </div>

                  <div className="w-full h-40 flex items-center justify-center my-4 overflow-hidden">
                    <img
                      src={cat.image}
                      alt={cat.name}
